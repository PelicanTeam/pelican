...
/**
 * The DrawVideo class allows the user to draw 3D markers with a background image.
 */
...
{ 
	 ...
	 */
	public Draw3D( Image inputImage ) 
	{ 
	 ...
	 */
	public Draw3D( Image inputImage, String title) 
	{ 
	 ...
	 *	@param markersImage 
	 */
	public Draw3D( Image inputImage, String title, IntegerImage markersImage ) 
	{ 
	 ...
	 */
	public Draw3D( Image _inputImage, String title, IntegerImage _markersImage, boolean transparencyEnabled, boolean addLabelsEnabled ) 
	{ 
		...
		this.output = new IntegerImage(inputImage.getXDim(),inputImage.getYDim(),inputImage.getZDim(),1,1);
		this.output.fill(0);
	...
	private void guiInitialization() 
	{
		...
		frameLbl = new JLabel(" Depth : 1/" + inputImage.getZDim() + "     ");
		frameBox.add(frameLbl);
		frameSld = new JSlider(SwingConstants.HORIZONTAL, 1, inputImage.getZDim(), 1);
		...
		{ 
			...
			{
				int z = frameSld.getValue() - 1;
				frameLbl.setText(" Depth : " + Integer.toString(frameSld.getValue()) + "/" + inputImage.getZDim() + "     ");
				bimg = fr.unistra.pelican.util.Tools.pelican2BufferedZ( inputImage, z );
				int vValue = scroll.getVerticalScrollBar().getValue();
				int hValue = scroll.getHorizontalScrollBar().getValue();
				display.set(bimg, (IntegerImage)output.getImage4D(z, Image.Z));
				checkButtons();
	...
	{
		markersHistory = new History[inputImage.getZDim()];

		for(int i = 0; i< inputImage.getZDim(); i++)
		{
	...
	public void checkButtons()
	{
		...
		if ( markersHistory[z].canForward() )
		{
	...
	public void saveMarker()
	{
		...
		for ( int x = 0 ; x < width ; x++ )
		{
			...
			{
				output.setPixelXYZBInt( x,y,frameSld.getValue()-1,0, (byte) raster.getSample( x,y,0 ) );
			}
		}
		...
		{ 
			...
			{ 
				...
				if(option == JOptionPane.OK_OPTION)
				{
					...
					frameSld.setValue(1);
					display.set( bimg);
					output = new IntegerImage(inputImage.getXDim(),inputImage.getYDim(),inputImage.getZDim(),1,1);
					output.fill(0);
					markerHistoryInit();
					checkButtons();
				}
	...
	{ 
		...
		{ 
			...
			{
				if(frameSld.getValue() < inputImage.getZDim())
				{
					frameSld.setValue(frameSld.getValue()+1);
				}
			}
