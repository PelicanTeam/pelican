 ...
 *
 */
public class ColorAlphaConnectivityCCOptimizedByLUT extends Algorithm{
	/**
	 * Image to process
	 */
	 ...
	 * 
	 */
	public ColorAlphaConnectivityCCOptimizedByLUT() 
	{
		super();
		...
		for(i=neighbourhood.length;--i>=0;)
		{
			...
			if(locX>=0&&locY>=0&&locZ>=0&&locT>=0&&locX<xDim&&locY<yDim&&locZ<zDim&&locT<tDim)
			{
				if(outputImage.getPixelXYZTInt(locX, locY, locZ, locT)==INITVALUE)
				{
					...
					//if(lut.get(Math.abs(pixelR - inputImage.getPixelByte(neighbourIndex)),Math.abs(pixelG - inputImage.getPixelByte(neighbourIndex+1)),Math.abs(pixelB - inputImage.getPixelByte(neighbourIndex+2)))<=alpha)
	...
	{
		return (IntegerImage)new ColorAlphaConnectivityCCOptimizedByLUT().process(inputImage,alpha,neighbourhood,lut);
	}
}
