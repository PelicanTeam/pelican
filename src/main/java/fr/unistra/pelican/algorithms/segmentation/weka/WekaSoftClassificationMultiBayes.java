...
public class WekaSoftClassificationMultiBayes extends Algorithm {
	...
	public void launch() throws AlgorithmException {
		...
		NaiveBayes[] classifier = new NaiveBayes[nbClassifiers];

		for (int i = 0; i < classifier.length; i++) {
			classifier[i] = new NaiveBayes();

			classifier[i].setUseKernelEstimator(true);
			classifier[i].setUseSupervisedDiscretization(false);

			// classifier.setCrossValidate(true);
			// classifier.setNoNormalization(true);
			// classifier[i].setDistanceWeighting(new
	...
	public static void main(String[] args) {
		...
		try {
			...
			Image work = (Image) new WekaSoftClassificationMultiBayes().process(source,
					samples);
