...
public class ColorLInfinitySmoothConnectivityCC extends Algorithm {
	 ...
	 * 
	 */
	public ColorLInfinitySmoothConnectivityCC() 
	{
		super();
	...
	{		
			...
			for(int i=neighboursWithCurrentLabel.size();--i>=0;)
			{
				...
				distanceG = Math.abs(pixelG - inputImage.getPixelByte(neighbourIndex+1));
				distanceB = Math.abs(pixelB - inputImage.getPixelByte(neighbourIndex+2));
	...
	{
		return (IntegerImage)new ColorLInfinitySmoothConnectivityCC().process(inputImage,alpha,neighbourhood);
	}
