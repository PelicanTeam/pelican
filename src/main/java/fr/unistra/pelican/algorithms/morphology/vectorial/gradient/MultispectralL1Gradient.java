 ...
 * L1 norm of the marginal gradient
 * 
 * @author Erchan Abdullah, Jonathan Weber
 * 
 */
public class MultispectralL1Gradient extends Algorithm {
	/**
	 * the input image
	 */
	...
	public static Image exec(Image inputImage, BooleanImage se) {
		return (Image) new MultispectralL1Gradient().process(inputImage, se);
	}
	 ...
	 * 
	 */
	public MultispectralL1Gradient() {
			...
			for ( int x = 0 ; x < inputImage.getXDim() ; x++ ) { 
				...
				for ( int b = 0 ; b < inputImage.getBDim() ; b++ ) 
					if ( inputImage.isPresent( x,y,z,t,b ) ) { 
						tmp += gradient.getPixelDouble(x, y, z, t, b);
					}
				if(tmp>1.)
					tmp=1.;
