...
/**
 * Perform a soft dilation with a flat structuring element and a weight map.
 ...
 * We take the 2th greater element of the sorted bag composed of one 1, two 5 and 3 two: {1,2,2,2,5,5} 
 * that is 5!
 * 
 ...
 */
public class SoftDilation extends Algorithm {
	/**
	 * Input image
	 */
	...
	/**
	 * Gray Int Structuring Element, describe both flat structuring element ( >0 values) and weight map (values)
	 */
	...
	private double getMinGray(int x,int y, int z,int t,int b, Point [] points) { 
		...
		for ( int i = 0 ; i < points.length ; i++ ) { 
         ...
         min=sb.getElementAt(Math.min(threshold-1, sb.size()-1)).doubleValue();
	...
	public static void main(String [] args)
	{
		...
		Viewer2D.exec(SoftDilation.exec(im, se, 1),"Dilation cross 31*31, threshold=1, normal dilation");
		Viewer2D.exec(SoftDilation.exec(im, se, 20),"Dilation cross 31*31, threshold=20");
		Viewer2D.exec(SoftDilation.exec(im, se, 80),"Dilation cross 31*31, threshold=40");
	...
	public static Image exec(Image image, GrayIntStructuringElement se, int threshold) {
		return (Image) new SoftDilation().process(image, se,threshold);
	}
}