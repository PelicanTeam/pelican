...
/**
 * Compute the addition of two images containing a hue band.
 * 
...
public class HueBasedColourAddition extends Algorithm {
	...
	/**
	 * Algorithm result: addition of input image one and two.
	 */
	public Image output;
...
//				output.setPixelXYBDouble(x, y, 0, Tools.hueAddition(p1[0], p2[0]));

				if (p1[1] + p2[1] > 1.0)
					output.setPixelXYBDouble(x, y, 1, 1.0);
				else
					output.setPixelXYBDouble(x, y, 1, p1[1] + p2[1]);

				if (p1[2] + p2[2] > 1.0)
					output.setPixelXYBDouble(x, y, 2, 1.0);
				else
	...
	public static Image exec(Image input1, Image input2) {
		return (Image) new HueBasedColourAddition().process(input1,
				input2);
	}
}
