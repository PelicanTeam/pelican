...
/**
 * This class represents a vector ordering scheme based on the cumulative canberra 
 * distances of the vectors in question.
 * 
 * @author E.A.
 *
 */

public class CanberraDistanceOrdering implements VectorialOrdering
{
	...
	private double distance(double[] p1,double[] p2)
	{
		...
		for(int i = 0; i < p1.length; i++){
			double tmp = Math.abs(p1[i] - p2[i]);
			double tmp2 = p1[i] + p2[i];
			dist += tmp/tmp2;
		}
