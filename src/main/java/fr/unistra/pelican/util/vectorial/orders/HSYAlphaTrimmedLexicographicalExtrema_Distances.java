...
public class HSYAlphaTrimmedLexicographicalExtrema_Distances implements VectorialOrdering,Comparator
{
	 ...
	 * @param alpha
	 */
	public HSYAlphaTrimmedLexicographicalExtrema_Distances(double alpha,double refHue)
	{
		if(alpha > 0.0){
			this.alpha = alpha;
			this.beta = alpha;
	 ...
	 * @param alpha
	 */
	public HSYAlphaTrimmedLexicographicalExtrema_Distances(double alpha,double beta,double refHue)
	{
	...
	public double[] min(double[][] p)
	{
		...
		for(int d = 2; d >= 1; d--){
			...
			int trimSize = 1;	// keep of course the minimum...
			
			// and add the vectors at distance at most alphaV[d] from it
			for(int i = 1; i < p.length; i++){
				if(Math.abs(p[i][d] - p[0][d]) <= alphaV[d]) trimSize++;
				else break;
			}
