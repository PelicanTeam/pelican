...
public class PathBasedOrderingAlphaTrimmed implements VectorialOrdering,Comparator
{	
	private int[][][] order = new int[256][256][256];
	
	public PathBasedOrderingAlphaTrimmed(Image img)
	{
		...
		for(int i = 0; i < pixels; i++){
			...
			int r = (int)Math.round(min.vector[0] * 255);
			int g = (int)Math.round(min.vector[1] * 255);
			int b = (int)Math.round(min.vector[2] * 255);
	...
	public VectorPixel getAlphaTrimmedMin(VectorPixel[] p,double alpha)
	{
		...
		for(int d = 0; d < dim - 1; d++){
			...
			for(int i = tmp; i < p.length; i++){
				if(p[i].vector[d] == p[tmp - 1].vector[d]) trimSize++;
				else break;
			}
		...
		// fill it
		for(int i = 0; i < ind.length; i++)
			ind[i] = new IndexedDouble(p[i].vector[dim - 1],i);
