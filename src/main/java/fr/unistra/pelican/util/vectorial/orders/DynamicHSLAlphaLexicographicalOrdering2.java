 ...
 * 
 * Here the value is computed in relation to a sigmoid...
 * 
...
public class DynamicHSLAlphaLexicographicalOrdering2 implements VectorialOrdering,Comparator
{
	 ...
	 */
	public DynamicHSLAlphaLexicographicalOrdering2(double alpha,int order,double refHue,double[][] histo)
	{
		...
		for(int i = 0; i < 256; i++){
			double prop = histo[1][i];
			double pixel = i;
			
			if(Tools.doubleCompare(prop,0.0) <= 0) transformed[i] = Math.ceil(pixel / alpha);
			else{
				double tmpAlpha = 1 / (1 + Math.exp(250.0 * (prop - avg)));
				
				double m = Math.ceil(pixel / alpha) - pixel / alpha;
				
				transformed[i] = Math.max(Math.ceil(pixel / alpha) - m * tmpAlpha,i>0?transformed[i - 1]:0.0);
			}
		}
	}
