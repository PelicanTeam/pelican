...
public class GlobalController extends JApplet {
...
//	}
	...
	/**
	 * Return the image defined by the parameter name.
	 * 
	 * @param name
	 * @return
	 */
	...
	public ArrayList<Object> inputAction(String algoName,
		...
		for (int i = 0; i < inputTypesList.length; i++) {
			...
			try {
				Class classe = Class.forName("fr.unistra.pelican.gui.application.inputs."
						+ classCalled);
		...
		// Demarcates the inputs from the options.
	...
	public void outputAction(Class[] outputTypesList,
		...
		for (int i = 0; i < outputList.size(); i++) {
			...
			try {
				Class classe = Class.forName("fr.unistra.pelican.gui.application.outputs."
						+ classCalled);
				Class[] methodTypeParameters = { GlobalController.class, String.class,
						Object.class };
